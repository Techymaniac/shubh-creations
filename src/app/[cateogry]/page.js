...
// NOTICE: We removed 'next/image' to use standard HTML tags
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        ...
        console.log("Searching for category:", rawCategory.toLowerCase());

        // THE FIX: Compare lowercase Database vs Lowercase URL
        const query = `*[_type == "product" && lower(category) == "${rawCategory.toLowerCase()}"]`;
        
        const data = await client.fetch(query);
        console.log("Found items:", data.length); // See if items return in console
        setProducts(data);
      } catch (error) {
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-sm text-gray-400 mt-2">
               Checked for "{params.category.toLowerCase()}".
            </p>
              ...
              <div key={item._id} className="bg-white rounded-lg overflow-hidden shadow-sm group hover:shadow-md transition-all">
                <Link href={`/product/${item._id}`}>
                  <div className="relative h-96 w-full cursor-pointer overflow-hidden bg-gray-100">
                    {/* BYPASS: Standard HTML img tag. No config needed. */}
                    {item.image && (
                      <img
